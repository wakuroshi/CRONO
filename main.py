...
def main():
    ...
    if args.local:
        ...
        try:
            ...
            disponibilidad_agentes = cargar_json(periodo_path / "disponibilidad_agentes.json")
            asignacion_agentes = cargar_json(periodo_path / "asignacion_agentes.json")
        ...
        try:
            with psycopg.connect(conn_string) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(f"SELECT payload FROM {TABLA_ENTIDADES} WHERE period_id = %s AND entity_type = 'disponibilidad_agentes';", (args.periodo,))
                    fila_disp = cur.fetchone()
                    if not fila_disp:
                        print(f"Error Crítico: No se encontró la entidad 'disponibilidad_agentes' para el periodo '{args.periodo}' en la base de datos.", file=sys.stderr)
                        sys.exit(1)
                    disponibilidad_agentes = fila_disp[0]

                    # Consulta 3: Asignacion de agentes (Nomenclatura Nueva)
                    cur.execute(f"SELECT payload FROM {TABLA_ENTIDADES} WHERE period_id = %s AND entity_type = 'asignacion_agentes';", (args.periodo,))
                    fila_asig = cur.fetchone()
                    if not fila_asig:
                        print(f"Error Crítico: No se encontró la entidad 'asignacion_agentes' para el periodo '{args.periodo}' en la base de datos.", file=sys.stderr)
                        sys.exit(1)
                    asignacion_agentes = fila_asig[0]

                    # Consulta 4: Estructuras (Mallas de grupos)
    ...
    resultados = solve_model(
        ...
        disponibilidad_agentes=disponibilidad_agentes,
        asignacion_agentes=asignacion_agentes,
        config=config,
                ...
                try:
                    with open(csv_path, mode='w', newline='', encoding='utf-8') as f_csv:
                        ...
                        for tarea in tareas:
                            sid = tarea["id"]
                            nombre = tarea["nombre"]
                            agente = tarea["agente"] if tarea["agente"] else "Ninguna"
                            ...
                            if tarea["horario"]:
                                ...
                                writer.writerow([grupo_name, subgrupo_id, sid, nombre, agente, recurso, "SIN_ASIGNAR", -1, -1])
