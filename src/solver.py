...
def solve_university_model(
    careers_data: Dict, availability: Dict, assignments: Dict, config: TimeConfig
):
        ...
        for sem in data["semesters"]:
            ...
            for sub in sem["subjects"]:
                ...
                if sid not in all_vars:
                    ...
                    sub_vars = [
                        model.NewBoolVar(f"v_{sid}_b{i}") for i in range(num_bloques)
                    ]
                    ...
                    if prof in availability:
                        ...
                        for i in range(num_bloques):
                            if i not in valid_indices:
                                model.Add(sub_vars[i] == 0).WithName(
                                    f"Disp_{prof}_{sid}_B{i}"
                                )
            ...
            for i in range(num_bloques):
                model.Add(sum(s_v[i] for s_v in sem_vars) <= 1).WithName(
                    f"NoChoque_Sem{sem_num}_B{i}"
                )
    ...
    for prof, vars_list in prof_map.items():
        for i in range(num_bloques):
            model.Add(sum(m_v[i] for m_v in vars_list) <= 1).WithName(
                f"NoChoqueProf_{prof}_B{i}"
            )
    ...
    for career_name, data in careers_data.items():
        for sem in data["semesters"]:
            ...
            for d_idx in range(len(config.dias)):
                day_start = d_idx * config.slots_por_dia
                dia_ocupado = model.NewBoolVar(
                    f"dia_ocupado_{career_name}_S{sem['number']}_d{d_idx}"
                )

                vars_dia = []
                for sub_vars in sem_subjects_vars:
                    vars_dia.extend(
                        sub_vars[day_start : day_start + config.slots_por_dia]
                    )
            ...
            total_dias_alumno = model.NewIntVar(
                0, 5, f"tot_dias_{career_name}_S{sem['number']}"
            )
            ...
            for d_idx in range(len(config.dias)):
                ...
                for i in range(day_start, day_start + config.slots_por_dia - 2):
                    ...
                    b_next = model.NewBoolVar(
                        f"occ_{career_name}_S{sem['number']}_b{i + 1}"
                    )
                    b_after = model.NewBoolVar(
                        f"occ_{career_name}_S{sem['number']}_b{i + 2}"
                    )
                    ...
                    ventana = model.NewBoolVar(
                        f"gap_{career_name}_S{sem['number']}_b{i}"
                    )
    ...
    solver.parameters.max_time_in_seconds = (
        60.0  # Se podria subir mas, no fue necesario en testeo
    )
